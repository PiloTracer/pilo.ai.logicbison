...
Until a dedicated `{BOUNDARY_MAP}` file exists, use **`.work/standards/*-DIRECTORY_MAP.md`** as the **interim** boundary list (`REPLACE:PLATFORM_PACKAGE/`, `<context-a>/`, `<context-b>/`, …). Promote a trimmed **edge-only** table here or into a dedicated boundary file when M1 lands and import lint exists.
