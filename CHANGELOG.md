...
### Added
- **Agent OS anchor helpers promoted into `scripts/sister-discovery.sh`** — `agent_os_names` + `find_agent_os_dir` (canonical implementation from `.ai.biz`'s extended variant): derive the orchestrator root from a family-named source by slot-strip (`pilo.ai.biz.logicbison` → `pilo.ai.logicbison`), fall back to well-known `pilo.ai.logicbison` then legacy `.ai`; exit 1 with empty output when none exists — the caller asks the user, never guesses. Unit-asserted in `framework-verify` smoke 2i (family-root derivation, resolution from family + legacy sources, empty-on-missing). Sibling alignment: `.ai.biz` (match lib) and `.ai.flutter` (replace 4 local `find_agent_os_dir` copies with the lib call) — directions in the session report.
...
[Unreleased]: https://github.com/PiloTracer/pilo.ai.logicbison/compare/v0.6.1...HEAD
[0.6.1]: https://github.com/PiloTracer/pilo.ai.logicbison/compare/v0.6.0...v0.6.1
