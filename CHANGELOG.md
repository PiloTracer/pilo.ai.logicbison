...
### Added
- **Operator handoff contract** — canonical response-closing contract in `skills/SKILL_DEPENDENCIES.md` (§ Operator handoff contract), implementing the operator-provided Response Clarity Protocol (`.work/prompts/improve-clarity-of-responses.md`): terse output; Form B close with separate labeled sections — `**Needs your approval:**` (decisions citing `path/to/file.md:L42`), `**Needs your answer:**` (numbered self-contained questions), `**Next step:**` (exactly one command) — empty sections omitted, decisions and questions never mixed; Form A single line when no operator input is needed. Applied to all 22 skills (reference line in every `skill.md`; every operator-facing report template closes with the handoff form; anti-pattern entries added where lists exist). Enforced mechanically: `scripts/skill-functional-verify.py` hard-fails any `skill.md` missing the contract reference. `.cursorrules` + `templates/cursorrules.template` § Verification & Communication synced.
...
### Changed
- **`session-control` commit scope is now repo-mode dependent** — supersedes the 0.6.0 `.work/`-only default. Mode detection: framework source repo ⇔ repo root contains both `templates/cursorrules.template` and `skills/session-control/skill.md`. Framework source: default `close commit` / `commit` stage the **whole repo** (all safe changed + untracked paths). Consumers: stage **`.work/` + root-level `PROCESS_ROUTER.md`, `DOCS_TECH_STACK.md`, `CHANGELOG.md` if present**. Both modes exclude secrets/`tmp/`/protected files; `commit scoped` (bookend files only) unchanged; `push` always ships the whole branch. Synced across `skills/session-control/skill.md` + `reference.md`, `.cursorrules`, `templates/cursorrules.template`, `skills/README.md`.
