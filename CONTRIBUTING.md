...
# Change-safety (run on dirty tree before commit in AI-assisted sessions)
bash scripts/touch-scope-verify.sh  # fail when diff exceeds .work/touch-scope or iteration Files
bash scripts/blast-radius-check.sh  # fail on risk: high; warn on risk: medium
...
## Change-safety gates (contributors)

Framework changes must pass three mechanical gates before commit:

| Gate | Command | What it checks |
|------|---------|----------------|
| Scope | `bash scripts/touch-scope-verify.sh --strict` | Diff stays within `.work/touch-scope` or iteration `Files` |
| Blast radius | `bash scripts/blast-radius-check.sh` | Cross-area risk, protected surface hits, line-count thresholds |
| MOD-06 | `bash scripts/mod06-output-check.sh <file>` | AI-assisted change has blast-radius paragraph + recommendation |

All three run via `@code-verify uncommitted` and `bash scripts/framework-verify.sh`. Fix failures with `@code-repair repair - from uncommitted`.
