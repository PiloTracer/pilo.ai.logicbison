...
| **spec** | "New feature SPEC", "Review SPEC", "Amend SPEC" | `@feature-spec create - <slug>` · `@feature-spec review - <path>` | `.ai/skills/feature-spec/skill.md` · `standards/*FEATURE_STANDARD*` §3 |
...
| **conventions** | "Naming", "type-check", "PR format" | Read section only | `standards/*CONVENTIONS*` (path from `.cursorrules`) |
| **security** | "Threat model", "New column classification" | Read section only | `standards/*threat-model*` · `*data-classification*` |
...
| **integration** | External API, vendor contract, signing | Read domain SPEC + integration mirror on demand | `{FEATURE_SPEC_ROOT}/<slug>/*-SPEC.md` · `docs/integration/MANIFEST.txt` |
