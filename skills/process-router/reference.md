...
| **spec** | "New feature SPEC", "Review SPEC", "Amend SPEC" | `@feature-spec create - <slug>` · `@feature-spec review - <path>` | `.ai/skills/feature-spec/skill.md` · `.work/standards/*FEATURE_STANDARD*` §3 |
...
| **conventions** | "Naming", "type-check", "PR format" | Read section only | `.work/standards/*CONVENTIONS*` (path from `.cursorrules`) |
| **security** | "Threat model", "New column classification" | Read section only | `.work/standards/*threat-model*` · `*data-classification*` |
...
| **integration** | External API, vendor contract, signing | Read domain SPEC + integration mirror on demand | `{FEATURE_SPEC_ROOT}/<slug>/*-SPEC.md` · `.work/docs/integration/MANIFEST.txt` |
